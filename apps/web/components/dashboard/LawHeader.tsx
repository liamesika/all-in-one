...
export function LawHeader() {
  ...
  return (
    <>
      <header
        className="fixed top-0 left-0 right-0 z-50 border-b border-amber-600/20 dark:border-gray-700 transition-all duration-300 backdrop-blur-sm"
        style={{
          background: 'linear-gradient(135deg, #0E1A2B 0%, #1a2942 100%)',
          boxShadow: '0 4px 12px -2px rgba(201, 162, 39, 0.15), 0 2px 6px -1px rgba(201, 162, 39, 0.1)',
          minHeight: '72px'
        }}
      >
        <div className="max-w-full mx-auto px-4 sm:px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div
              ...
              onClick={() => router.push('/dashboard/law/dashboard')}
            >
              <Image
            ...
            <nav className="hidden md:flex items-center gap-8">
              {navItems.map((item) => (
                <button
                  ...
                  className="relative text-white/90 hover:text-amber-300 font-medium text-base transition-all duration-300 group"
                >
                  <span className="relative z-10">{item.label}</span>
                  <span
                    className="absolute inset-x-0 -bottom-1 h-0.5 bg-amber-400 scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-center"
                  />
                </button>
              ))}
            </nav>
